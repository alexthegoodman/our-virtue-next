...
async function performCustomModerationCheck(content: string): Promise<ModerationResult> {
  try {
    if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === 'your-openai-api-key-here') {
      return { isAppropriate: true, confidence: 0 };
    }
...
export function checkRateLimit(userId: string, windowMs: number = 60000, maxPosts: number = 5): boolean {
  const now = Date.now();
...
export function detectSpam(content: string): { isSpam: boolean; reason?: string } {
  ...
  const spamPhrases = [
    ...
    'viagra', 'casino', 'lottery', 'winner', 'congratulations you',
  ];
